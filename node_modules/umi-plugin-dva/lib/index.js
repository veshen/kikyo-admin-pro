...
function _default(api, opts = {}) {
  ...
  function getGlobalModelContent() {
    return exclude(getGlobalModels(api, shouldImportDynamic), (0, _utils.optsToArray)(opts.exclude)).map(path => `
  ...
  if (shouldImportDynamic) {
    api.modifyRouteComponent((memo, args) => {
      ...
      if (opts.dynamicImport.loadingComponent) {
        loadingOpts = `LoadingComponent: require('${(0, _slash.default)((0, _path.join)(paths.absSrcPath, opts.dynamicImport.loadingComponent))}').default,`;
      }
...
app: window.g_app,
models: () => [
  ${models.map(model => `import(${opts.dynamicImport.webpackChunkName ? `/* webpackChunkName: '${(0, _utils.chunkName)(paths.cwd, model)}' */` : ''}'${model}')`).join(',\r\n  ')}
],
      `.trim());
      }
