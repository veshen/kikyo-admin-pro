...
function _default(opts) {
  ...
  .add((0, _path.join)(__dirname, '../../../')).end().extensions.merge(['.web.js', '.mjs', '.js', '.web.jsx', '.jsx', '.web.ts', '.ts', '.web.tsx', '.tsx', '.json']);
  ...
  const rule = webpackConfig.module.rule('exclude').exclude.add(/\.json$/).add(/\.(js|jsx|ts|tsx|mjs)$/).add(/\.(css|less|scss|sass)$/);
  ...
  rule.end().use('url-loader').loader(require.resolve('url-loader')).options({
  ...
  webpackConfig.module.rule('ts').test(/\.tsx?$/).include.add(opts.cwd).end().exclude.add(/node_modules/).end().use('babel-loader').loader(require.resolve('babel-loader')).options(babelOpts).end().use('ts-loader').loader(require.resolve('ts-loader')).options(_objectSpread({
