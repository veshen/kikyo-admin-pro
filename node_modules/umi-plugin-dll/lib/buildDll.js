...
var _lodash = _interopRequireDefault(require("lodash.pullall"));
...
function _default(opts = {}) {
  ...
  const depNames = (0, _lodash.default)((0, _lodash2.default)(Object.keys(pkg.dependencies || {}).concat(include || [])), exclude).filter(dep => {
    return dep !== 'umi' && !dep.startsWith('umi-plugin-');
  });

  const webpack = require(_resolveDeps('af-webpack/webpack'));

  const files = (0, _lodash2.default)([...depNames, 'umi/link', 'umi/dynamic', 'umi/navlink', 'umi/redirect', 'umi/router', 'umi/withRouter', 'umi/_renderRoutes', 'umi/_createHistory', 'react', 'react-dom', 'react-router-dom']).sort((a, b) => a > b ? 1 : -1);
