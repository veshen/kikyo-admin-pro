...
function _default(webpackConfig, opts) {
  ...
  const postcssOptions = {
    ...
    (0, _autoprefixer.default)({
      ...
      flexbox: 'no-2009'
    }), ...(opts.extraPostCSSPlugins ? opts.extraPostCSSPlugins : []), ...(isDev || process.env.CSS_COMPRESS === 'none' || process.env.COMPRESS === 'none' || process.env.NO_COMPRESS ? [] : [require('cssnano')({
  ...
  if (opts.cssModulesExcludes) {
    opts.cssModulesExcludes.forEach((exclude, index) => {
      ...
      const config = webpackConfig.module.rule(rule).test(filePath => filePath.indexOf(exclude) > -1);
